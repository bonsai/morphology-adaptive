...
$LogicFile = Join-Path $PublicDir "game_logic.py"
...
if ($hasDocker) {
    ...
    docker run --rm -v "${ProjectRoot}:/build" -w /build morphology-builder wasm-pack build --target web --out-dir public/pkg
} else {
...
Write-Host "1. Rust WASM (Fastest, loaded from public/pkg/)"
Write-Host "2. Python/Pyodide (Fallback, loaded from public/game_logic.py)"
...
Write-Host "http://localhost:8000/public/index.html"
